...
public class ProductDao {
    ...
    private static final String addProductSql = "insert into products(prod_name, cost, descr, count, type) values(?, ?, ?, ?, ?)";
	...
	try (Connection conn = DbConnFactory.getConnection();
		PreparedStatement preparedStatement = conn.prepareStatement(addProductSql)) {
	    ...
	    preparedStatement.setInt(4, prod.getCount());
	    preparedStatement.setString(5, prod.getType());
	...
	try (Connection connection = DbConnFactory.getConnection();
		PreparedStatement preparedStatement = connection
			.prepareStatement(getProductByIdSql)) {
	...
	try (Connection conn = DbConnFactory.getConnection();
		PreparedStatement preparedStatement = conn.prepareStatement(getProductByTypeSql)) {
	...
	try (Connection conn = DbConnFactory.getConnection();
		PreparedStatement preparedStatement = conn.prepareStatement(updateProductSql)) {
	    preparedStatement.setString(1, prod.getProd_name());
	    preparedStatement.setInt(2, prod.getCost());
	    preparedStatement.setString(3, prod.getDesc());
	...
	try (Connection conn = DbConnFactory.getConnection();
		PreparedStatement preparedStatement = conn.prepareStatement(getAllProductsSql)) {
	    resultSet = preparedStatement.executeQuery();
